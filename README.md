...
- Read secrets **from environment variables only** (`OPENAI_API_KEY`, `GEMINI_API_KEY`, …). Copy `.env.example` → `.env` (already gitignored) if you keep one. **Never hardcode keys.**
